<script>
	...
	function redirection_letter(letter)
	{
		...
		else
		{
			if({REWRITE})
			{
                cat = str_cat ? "-" + str_cat.substring(1) : "";
				location.href="{PATH_TO_ROOT}/dictionary/dictionary-" + letter + cat + ".php";
			}
			else
			{
                cat = str_cat ? "&cat=" + str_cat.substring(1) : "";
				location.href="{PATH_TO_ROOT}/dictionary/dictionary.php?l=" + letter + cat;
			}
		}
	}
