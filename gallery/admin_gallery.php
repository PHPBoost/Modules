...
if ($nbr_pics > 0)
{
    ...
    else
    {
        ...
        while ($row = $result->fetch())
        {
            ...
            $name_cut = (TextHelper::strlen(TextHelper::html_entity_decode($name)) > 22) ? TextHelper::htmlspecialchars(TextHelper::substr(TextHelper::html_entity_decode($name), 0, 22)) . '...' : $name;
            ...
            $view->assign_block_vars('pics.list', [
                ...
                'C_AUTHOR_GROUP_COLOR' => !empty($group_color),
                'C_FULLSCREEN'         => $config->get_pics_enlargement_mode() == GalleryConfig::FULL_SCREEN,
