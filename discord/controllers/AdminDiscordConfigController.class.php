...
class AdminDiscordConfigController extends DefaultAdminModuleController
{
    public function execute(HTTPRequestCustom $request): DefaultAdminDisplayResponse
    {
        $this->build_form();
    ...
    private function build_form(): void
    {
        ...
        $fieldset->add_field(new FormFieldNumberEditor('discord_id', $this->lang['discord.id'], $this->config->get_discord_id(),
            ['description' => $this->lang['discord.id.clue']]
        ));

        $fieldset_authorizations = new FormFieldsetHTML('authorizations', $this->lang['form.authorizations']);
        $form->add_fieldset($fieldset_authorizations);

        $auth_settings = new AuthorizationsSettings([new ActionAuthorization($this->lang['form.authorizations.read'], DiscordAuthorizationsService::READ_AUTHORIZATIONS)]);
    ...
    {
        $this->config->set_discord_id($this->form->get_value('discord_id'));
