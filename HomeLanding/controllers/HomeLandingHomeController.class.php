...
class HomeLandingHomeController extends ModuleController
{
	...
	private function build_articles_cat_view()
	{
		...
		while ($row = $result->fetch())
		{
			...
			$description = trim(mb_substr($short_contents, 0, $nb_char));
			$cut_contents = trim(mb_substr($contents, 0, $nb_char));
			
			$tpl->assign_block_vars('item', array_merge($article->get_tpl_vars(), array(
	...
	private function build_events_view()
	{
		...
		{			
			...
			$description = mb_substr(@strip_tags(FormatingHelper::second_parse($row['contents']), '<br><br/>'), 0, $this->modules[HomeLandingConfig::MODULE_CALENDAR]->get_characters_number_displayed());
			
			$tpl->assign_block_vars('events_items', array_merge($event->get_array_tpl_vars(), array(
				'C_READ_MORE' => mb_strlen(FormatingHelper::second_parse($row['contents'])) >= $this->modules[HomeLandingConfig::MODULE_CALENDAR]->get_characters_number_displayed(),
				'DESCRIPTION' => $description
			)));			
		}
		$result->dispose();
...
';
	...
	private function build_forum_view()
	{
		...
		while ($row = $result->fetch())
		{
			...
			$tpl->assign_block_vars('forum_items', array(
				...
				'CONTENTS' => stripcslashes(trim(mb_substr($contents, 0, $nb_char))),
	...
	private function build_media_view()
	{
		...
		while ($row = $result->fetch())
		{
			...
			elseif ($mime_type_tpl == 'video/x-flv' OR $mime_type_tpl == 'video/mp4')
			{
		...
		else
		{
			...
			for($i = 0; $i < $nbr_item ; $i++)
			{
				...
				$tpl->assign_block_vars('rssreader',array(
					...
					'DATE_FEED' => $date_feed,
					'DESC' => strip_tags(trim(mb_substr($desc, 0, $nb_char))),
