...
SmalladsFormFieldCarousel.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : '', class : 'grouped-element', 'aria-label' : '{@upload.files.management}', onclick : "window.open('{PATH_TO_ROOT}/user/upload.php?popup=1&fd=field_picture_url_" + id + "&parse=true&no_path=true', '', 'height=500,width=780,resizable=yes,scrollbars=yes');return false;"}).html('<i class="fa fa-cloud-upload-alt" aria-hidden="true"></i>').appendTo('#' + id);
			jQuery('#' + id).append(' ');
