...
SmalladsFormFieldBrand.prototype = {
	add_field : function () {
		if (this.integer <= this.max_input) {
			...
			jQuery('<a/> ', {href : 'javascript:SmalladsFormFieldBrand.delete_field('+ this.integer +');', 'aria-label' : ${escapejs(LangLoader::get_message('delete', 'common'))}}).html('<i class="fa fa-trash-alt" aria-hidden="true"></i> <span class="sr-only">' + ${escapejs(LangLoader::get_message('delete', 'common'))} + '</span>').appendTo('#' + id);
...
# START fieldelements #
		<div id="${escape(ID)}_{fieldelements.ID}">
			...
			<a href="javascript:SmalladsFormFieldBrand.delete_field({fieldelements.ID});" data-confirmation="delete-element" aria-label="${LangLoader::get_message('delete', 'common')}"><i class="fa fa-trash-alt" aria-hidden="true"></i><span class="sr-only">${escapejs(LangLoader::get_message('delete', 'common'))}</span></a>
		</div>
# END fieldelements #
</div>
