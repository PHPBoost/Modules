 ...
 * @version   	PHPBoost 5.2 - last update: 2019 04 09
...
class SmalladsSetup extends DefaultModuleSetup
{
	...
	private function change_fields()
	{
		...
		foreach ($rows_change as $old_name => $new_name)
		{
			if (isset($this->columns[$old_name]))
				PersistenceContext::get_querier()->inject('ALTER TABLE ' . PREFIX . 'smallads CHANGE ' . $old_name . ' ' . $new_name);
		}
	}
	private function delete_files()
	{
		...
		$file->delete();
		$file = new File(PATH_TO_ROOT . '/smallads/lang/english/smallads_english.php');
		$file->delete();
