 ...
 * @version     PHPBoost 6.0 - last update: 2025 09 25
...
class VideoItem
{
	...
	{
		return CategoriesService::get_categories_manager('video')->get_categories_cache()->get_category($this->id_category);
	}
	...
	public function get_real_summary()
	{
		...
		return TextHelper::cut_string(@strip_tags(FormatingHelper::second_parse($this->content), '<br><br/>'), (int)VideoConfig::load()->get_auto_cut_characters_number());
	}
	...
	public function set_properties(array $properties)
	{
		...
		$notation->set_module_name('video');
	...
	{
		$this->id_category = $id_category;
        $this->content = VideoConfig::load()->get_default_content();
	...
	public function get_item_url()
	{
		...
		return VideoUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $this->id, $this->rewrited_title)->rel();
	}
	...
	public function get_template_vars()
	{
		...
		$rich_content = HooksService::execute_hook_display_action('video', $content, $this->get_properties());
		$real_summary = $this->get_real_summary();
		$user = $this->get_author_user();
		$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
		$comments_number = CommentsService::get_comments_number('video', $this->id);
		...
		return array_merge(
			...
			array(
				...
				'C_NEW_CONTENT'          => ContentManagementConfig::load()->module_new_content_is_enabled_and_check_date('video', $this->get_publishing_start_date() != null ? $this->get_publishing_start_date()->get_timestamp() : $this->get_creation_date()->get_timestamp()) && $this->is_published(),
				...
				'L_COMMENTS'      => CommentsService::get_lang_comments('video', $this->id),
				...
				'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? VideoUrlBuilder::configuration()->rel() : CategoriesUrlBuilder::edit($category->get_id(), 'video')->rel(),
