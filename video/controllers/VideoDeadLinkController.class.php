...
class VideoDeadLinkController extends AbstractController
{
	private $item;
	...
	public function execute(HTTPRequestCustom $request)
	{
		...
		{
			if (!PersistenceContext::get_querier()->row_exists(PREFIX . 'events', 'WHERE id_in_module=:id_in_module AND module=\'video\' AND current_status = 0', array('id_in_module' => $this->item->get_id())))
			{
				...
				$contribution->set_fixing_url(VideoUrlBuilder::edit($this->item->get_id())->relative());
