...
class VideoMemberItemsController extends DefaultModuleController
{
	...
	{
		$now = new Date();
        $result = PersistenceContext::get_querier()->select('SELECT video.*, member.*
            ...
            ORDER BY member.display_name, video.creation_date DESC', [
            ...
            foreach (array_unique($contributors) as $user_id)
            {
                ...
                if ($user)
                {
                    $this->view->assign_block_vars('users', [
                        ...
                        'U_USER' => VideoUrlBuilder::display_member_items($user->get_id())->rel(),
	...
	{
		$page = $request->get_getint('page', 1);
		$page_title = $this->member ? ($this->is_current_member_displayed() ? $this->lang['video.my.items'] : $this->get_member()->get_display_name()) : $this->lang['contribution.members.list'];
		...
		if ($this->member)
        {
            $graphical_environment->get_seo_meta_data()->set_description(StringVars::replace_vars($this->lang['video.seo.description.member'], ['author' => $this->get_member()->get_id()]));
        ...
        else
        {
            ...
            $graphical_environment->get_seo_meta_data()->set_canonical_url(VideoUrlBuilder::display_member_items());
        }

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['video.module.title'], VideoUrlBuilder::home());
		$breadcrumb->add($this->lang['contribution.members.list'], VideoUrlBuilder::display_member_items());
        if ($this->member)
