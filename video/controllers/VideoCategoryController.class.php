...
class VideoCategoryController extends DefaultModuleController
{
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$sort_options = [
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.update'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_UPDATE_DATE], ['data_option_icon' => 'far fa-calendar-plus']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.date'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_DATE], ['data_option_icon' => 'far fa-calendar-alt']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.alphabetic'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_ALPHABETIC], ['data_option_icon' => 'fa fa-sort-alpha-up']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.author'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_AUTHOR], ['data_option_icon' => 'fa fa-user']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.views.number'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_VIEWS_NUMBERS], ['data_option_icon' => 'far fa-eye'])
		];

		if ($this->comments_config->module_comments_is_enabled('video'))
			$sort_options[] = new FormFieldSelectChoiceOption($this->lang['common.sort.by.comments.number'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_COMMENTS_NUMBER], ['data_option_icon' => 'far fa-comments']);

		if ($this->content_management_config->module_notation_is_enabled('video'))
			$sort_options[] = new FormFieldSelectChoiceOption($this->lang['common.sort.by.best.note'], VideoItem::SORT_FIELDS_URL_VALUES[VideoItem::SORT_NOTATION], ['data_option_icon' => 'far fa-star']);

		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			['select_to_list' => true, 'events' => ['change' => 'document.location = "'. VideoUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();']]
		));
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['video.seo.description.root'], ['site' => GeneralConfig::load()->get_site_name()]) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '');
