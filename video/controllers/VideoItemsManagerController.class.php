...
class VideoItemsManagerController extends DefaultModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(VideoSetup::$video_table, 'items-manager', $columns, new HTMLTableSortingRule('creation_date', HTMLTableSortingRule::DESC));

		$table_model->set_layout_title($this->lang['video.items.management']);

		$table_model->set_filters_menu_title($this->lang['video.filter.items']);
		...
		foreach ($result as $row)
		{
			...
			$edit_link = new EditLinkHTMLElement(VideoUrlBuilder::edit($item->get_id()));
			...
			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(VideoUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_title()), 'left'),
        ...
        if ($request->get_string('delete-selected-elements', false))
        {
            for ($i = 1; $i <= $this->elements_number; $i++)
            {
                if ($request->get_value('delete-checkbox-' . $i, 'off') == 'on')
                {
                    if (isset($this->ids[$i]))
                    {
						$item = VideoService::get_item($this->ids[$i]);
                        VideoService::delete($this->ids[$i]);
            ...
            AppContext::get_response()->redirect(VideoUrlBuilder::manage(), $this->lang['warning.process.success']);
        }
    }
	...
	private function generate_response($page = 1)
	{
		...
		$graphical_environment->set_page_title($this->lang['video.items.management'], $this->lang['video.module.title'], $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(VideoUrlBuilder::manage());

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['video.module.title'], VideoUrlBuilder::home());

		$breadcrumb->add($this->lang['video.items.management'], VideoUrlBuilder::manage());
