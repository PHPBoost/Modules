...
class AdminVideoConfigController extends DefaultAdminModuleController
{
	...
	private function build_form()
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('display_type', $this->lang['form.display.type'], $this->config->get_display_type(),
			[
				new FormFieldSelectChoiceOption($this->lang['form.display.type.grid'], VideoConfig::GRID_VIEW, ['data_option_icon' => 'fa fa-th-large']),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.list'], VideoConfig::LIST_VIEW, ['data_option_icon' => 'fa fa-list']),
				new FormFieldSelectChoiceOption($this->lang['form.display.type.table'], VideoConfig::TABLE_VIEW, ['data_option_icon' => 'fa fa-table'])
			],
			[
				'select_to_list' => true,
				'events' => ['change' => '
		...
		$fieldset->add_field(new VideoFormFieldExtension('authorized_extensions', $this->lang['video.authorized.extensions'], $this->config->get_mime_type_list(),
			['class' => 'full-field', 'description' => $this->lang['video.authorized.extensions.clue']]
		));
		...
		$sort_options = [
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.update'], VideoItem::SORT_UPDATE_DATE, ['data_option_icon' => 'far fa-calendar-plus']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.date'], VideoItem::SORT_DATE, ['data_option_icon' => 'far fa-calendar-alt']),
			new FormFieldSelectChoiceOption($this->lang['common.sort.by.views.number'], VideoItem::SORT_VIEWS_NUMBERS, ['data_option_icon' => 'fa fa-eye']),
		];

		if ($this->content_management_config->module_notation_is_enabled('video'))
			$sort_options[] = new FormFieldSelectChoiceOption($this->lang['common.sort.by.best.note'], VideoItem::SORT_NOTATION, ['data_option_icon' => 'far fa-star']);
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_type', $this->lang['common.sort.direction'], $this->config->get_sort_type(), $sort_options,
			['select_to_list' => true, 'description' => $this->lang['video.config.sort.type.clue']]
		));
