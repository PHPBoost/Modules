# IF C_ITEMS #
	<script>
		...
		var WikiItem = function(id, wiki){
			this.id = id;
			this.WikiItems = wiki;

			if (WikiItems.items_number > 1)
				WikiItems.change_reposition_pictures();
		};

		var WikiItems = new WikiItems('items-list');
		jQuery(document).ready(function() {
			WikiItems.init_sortable();
		});
	</script>
# ENDIF #
...
<section id="module-wiki">
	<header class="section-header">
		<div class="controls align-right">
			<a class="offload" href="${relative_url(SyndicationUrlBuilder::rss('wiki', CATEGORY_ID))}" aria-label="{@common.syndication}"><i class="fa fa-rss warning" aria-hidden="true"></i></a>
	...
	# IF C_ITEMS #
		<div class="sub-section">
			<div class="content-container">
				<div class="content">
					<form action="{REWRITED_SCRIPT}" method="post" id="position-update-form" onsubmit="WikiItems.serialize_sortable();" class="wiki-reorder-form">
							...
							<ul id="items-list" class="sortable-block">
								# START items #
									<li class="sortable-element# IF items.C_NEW_CONTENT # new-content# ENDIF #" id="list-{items.ID}" data-id="{items.ID}">
										...
										<script>
											jQuery(document).ready(function() {
												var wiki_question = new WikiItem({items.ID}, WikiItems);
