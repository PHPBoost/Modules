...
if (!empty($idm_get) && $del) //Suppression d'un message/topic.
{
    ...
    {
        if (!empty($msg['idtopic']) && (ForumAuthorizationsService::check_authorizations($topic['id_category'])->moderation() || AppContext::get_current_user()->get_id() == $topic['user_id']))
        {
    ...
    {
        if (!empty($topic['id_category']) && (ForumAuthorizationsService::check_authorizations($topic['id_category'])->moderation() || AppContext::get_current_user()->get_id() == $msg['user_id']))
        {
...
elseif (!empty($idt_get))
{
	...
	//On va chercher les infos sur le topic
    try {
        $topic = PersistenceContext::get_querier()->select_single_row(PREFIX . 'forum_topics', ['user_id', 'id_category', 'title', 'subtitle', 'nbr_msg', 'last_msg_id', 'first_msg_id', 'last_timestamp', 'status', 'display_msg'], 'WHERE id=:id', ['id' => $idt_get]);
    } catch (RowNotFoundException $e) {
        $error_controller = PHPBoostErrors::unexisting_element();
        DispatchManager::redirect($error_controller);
    }
...
elseif (!empty($track) && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL))
{
...
elseif (!empty($track_pm) && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL))
{
    AppContext::get_session()->csrf_get_protect();
...
elseif (!empty($untrack_pm) && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL))
{
    AppContext::get_session()->csrf_get_protect();
...
elseif (!empty($track_mail) && AppContext::get_current_user()->check_level(User::MEMBER_LEVEL))
{
    AppContext::get_session()->csrf_get_protect();
...
// -------------------------------------------------------------
// 4. Marquer les sujets comme lus
// -------------------------------------------------------------
...
elseif (!empty($selected) && !empty($idm_get)) //Sélection de la meilleure réponse
{
    AppContext::get_session()->csrf_get_protect();
    ...
    if ($topic['user_id'] == AppContext::get_current_user()->get_id() || AppContext::get_current_user()->check_level(User::MODERATOR_LEVEL))
    {
        if ($selected === "true")
        {
            PersistenceContext::get_querier()->update(ForumSetup::$forum_message_table, ['selected' => 1], 'WHERE id=:id', ['id' => $idm_get]);
            AppContext::get_response()->redirect('/forum/topic' . url('.php?id=' . $message_selected['idtopic'], '-' . $message_selected['idtopic'] . '.php', '&'));
        }
        else if ($selected === "false")
        {
            PersistenceContext::get_querier()->update(ForumSetup::$forum_message_table, ['selected' => 0], 'WHERE id=:id', ['id' => $idm_get]);
            AppContext::get_response()->redirect('/forum/topic' . url('.php?id=' . $message_selected['idtopic'], '-' . $message_selected['idtopic'] . '.php', '&'));
        }
        else
        {
