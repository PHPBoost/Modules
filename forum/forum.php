...
foreach ($categories as $id => $cat)
{
	if ($cat->get_id() != Category::ROOT_CATEGORY)
	{
		if ($cat->get_type() == ForumCategory::TYPE_FORUM)
			$Bread_crumb->add($cat->get_name(), 'forum' . url('.php?id=' . $cat->get_id(), '-' . $cat->get_id() . '-1-' . $cat->get_rewrited_name() . '.php'));
		else
...
if (!empty($id_get))
{
	...
	while ($row = $result->fetch())
	{
		...
		$view->assign_block_vars('topics', array_merge(
			...
			'U_TOPIC'             => url('.php?id=' . $row['id'], '-' . $row['id'] . '-1' . $rewrited_title_topic . '.php'),
