 ...
 * @version     PHPBoost 6.0 - last update: 2024 01 27
 * @since       PHPBoost 6.0 - 2021 08 22
*/

class SpotsFeedProvider implements FeedProvider
{
	public function get_feeds_list()
	{
		return CategoriesService::get_categories_manager('spots')->get_feeds_categories_module()->get_feed_list();
	}
	...
	public function get_feed_data_struct($idcat = 0, $name = '')
	{
		...
		if (CategoriesService::get_categories_manager($module_id)->get_categories_cache()->category_exists($idcat))
		{
			...
			$feed_module_name = LangLoader::get_message('spots.module.title', 'common', 'spots');
			$data = new FeedData();
			$data->set_title($feed_module_name . ' - ' . $site_name);
			$data->set_date(new Date());
			$data->set_link(SyndicationUrlBuilder::rss('spots', $idcat));
			$data->set_host(HOST);
			...
			$now = new Date();
			$results = $querier->select('SELECT spots.id, spots.id_category, spots.title, spots.rewrited_title, spots.content, spots.creation_date, spots.thumbnail, cat.rewrited_name AS rewrited_name_cat
			...
			foreach ($results as $row)
			{
				...
				$link = SpotsUrlBuilder::display($row['id_category'], $row['rewrited_name_cat'], $row['id'], $row['rewrited_title']);
