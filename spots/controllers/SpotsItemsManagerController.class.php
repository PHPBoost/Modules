...
class SpotsItemsManagerController extends DefaultAdminModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(SpotsSetup::$spots_table, 'table', [
		...
		foreach ($result as $row)
		{
			...
			$edit_item = new LinkHTMLElement(SpotsUrlBuilder::edit($item->get_id()), '', ['title' => $this->lang['common.edit']], 'fa fa-edit');
			$delete_item = new LinkHTMLElement(SpotsUrlBuilder::delete($item->get_id()), '', ['title' => $this->lang['common.delete'], 'data-confirmation' => 'delete-element'], 'far fa-trash-alt');
			...
			$row = [
				new HTMLTableRowCell(new LinkHTMLElement(SpotsUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_title()), 'align-left'),
				new HTMLTableRowCell(new LinkHTMLElement(
					...
					['data-color-surround' => $category->get_id() != Category::ROOT_CATEGORY ? $category->get_color() : SpotsConfig::load()->get_default_color()], 'pinned')
				),
