...
class SpotsItem
{
	...
	public function get_template_vars()
	{
		...
		{
			$category_address = $root_category ? GoogleMapsConfig::load()->get_default_marker_address() : $category->get_category_address();
		}
		$category_address_values = TextHelper::deserialize($category_address);
        ...
        return array_merge(
			...
			[
                ...
                'CATEGORY_LATITUDE'    => 
					$root_category && ModulesManager::is_module_activated('GoogleMaps')
                        ? GoogleMapsConfig::load()->get_default_marker_latitude()
                        : (
							is_array($category_address_values) && isset($category_address_values['latitude'])
								? $category_address_values['latitude']
                                : 0
                        ),
                'CATEGORY_LONGITUDE'   => 
					$root_category && ModulesManager::is_module_activated('GoogleMaps')
                        ...
                        : (
							is_array($category_address_values) && isset($category_address_values['longitude'])
							? $category_address_values['longitude']
                            : 0
                        ),
