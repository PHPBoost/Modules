		...
		if ($config->is_pm_for_members_birthday_enabled())
		{
			...
			foreach ($users as $user)
			{
				...
				PrivateMsg::start_conversation(
					$user['user_id'],
					StringVars::replace_vars($config->get_pm_for_members_birthday_title(), ['user_display_name' => $user['display_name'], 'user_age' => $user['age']]),
					StringVars::replace_vars($config->get_pm_for_members_birthday_content(), ['user_display_name' => $user['display_name'], 'user_age' => $user['age']]),
					'-1',
