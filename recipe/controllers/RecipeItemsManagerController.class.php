...
class RecipeItemsManagerController extends DefaultModuleController
{
	...
	private function build_table()
	{
		...
		$table_model = new SQLHTMLTableModel(RecipeSetup::$recipe_table, 'items-manager', $columns, new HTMLTableSortingRule('creation_date', HTMLTableSortingRule::DESC));

		$table_model->set_layout_title($this->lang['recipe.items.management']);

		$table_model->set_filters_menu_title($this->lang['recipe.filter.items']);
		...
		$results = array();
		$result = $table_model->get_sql_results('recipe
			...
			LEFT JOIN ' . DB_TABLE_NOTE . ' note ON note.id_in_module = recipe.id AND note.module_name = \'recipe\' AND note.user_id = ' . AppContext::get_current_user()->get_id() . '
		...
		foreach ($result as $row)
		{
			...
			$edit_link = new EditLinkHTMLElement(RecipeUrlBuilder::edit($item->get_id()));
			$delete_link = new DeleteLinkHTMLElement(RecipeUrlBuilder::delete($item->get_id()));

			$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
			$author = $user->get_id() !== User::VISITOR_LEVEL ? new LinkHTMLElement(UserUrlBuilder::profile($user->get_id()), $user->get_display_name(), (!empty($user_group_color) ? array('style' => 'color: ' . $user_group_color) : array()), UserService::get_level_class($user->get_level())) : $user->get_display_name();

			$row = array(
				new HTMLTableRowCell(new LinkHTMLElement(RecipeUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), $item->get_title()), 'align-left'),
				new HTMLTableRowCell(new LinkHTMLElement(RecipeUrlBuilder::display_category($category->get_id(), $category->get_rewrited_name()), ($category->get_id() == Category::ROOT_CATEGORY ? $this->lang['common.none.alt'] : $category->get_name()))),
        ...
        if ($request->get_string('delete-selected-elements', false))
        {
            for ($i = 1; $i <= $this->elements_number; $i++)
            {
                if ($request->get_value('delete-checkbox-' . $i, 'off') == 'on')
                {
                    if (isset($this->ids[$i]))
                    {
						$item = RecipeService::get_item($this->ids[$i]);
                        RecipeService::delete($this->ids[$i]);
            ...
            AppContext::get_response()->redirect(RecipeUrlBuilder::manage(), $this->lang['warning.process.success']);
        }
    }
	...
	private function generate_response($page = 1)
	{
		...
		$graphical_environment->set_page_title($this->lang['recipe.items.management'], $this->lang['recipe.module.title'], $page);
		$graphical_environment->get_seo_meta_data()->set_canonical_url(RecipeUrlBuilder::manage());

		$breadcrumb = $graphical_environment->get_breadcrumb();
		$breadcrumb->add($this->lang['recipe.module.title'], RecipeUrlBuilder::home());

		$breadcrumb->add($this->lang['recipe.items.management'], RecipeUrlBuilder::manage());
