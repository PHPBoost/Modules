...
class RecipeCategoryController extends DefaultModuleController
{
	...
	private function build_sorting_form($field, $mode)
	{
		...
		$fieldset->add_field(new FormFieldSimpleSelectChoice('sort_fields', '', $field, $sort_options,
			['select_to_list' => true, 'events' => ['change' => 'document.location = "'. RecipeUrlBuilder::display_category($this->category->get_id(), $this->category->get_rewrited_name())->rel() .'" + HTMLForms.getField("sort_fields").getValue() + "/" + HTMLForms.getField("sort_mode").getValue();']]
		));
	...
	private function generate_response(HTTPRequestCustom $request)
	{
		...
		if (empty($description))
			$description = StringVars::replace_vars($this->lang['recipe.seo.description.root'], ['site' => GeneralConfig::load()->get_site_name()]) . ($this->get_category()->get_id() != Category::ROOT_CATEGORY ? ' ' . $this->lang['category.category'] . ' ' . $this->get_category()->get_name() : '');
