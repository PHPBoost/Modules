...
class RecipeItemFormController extends DefaultModuleController
{
		...
		if (CategoriesAuthorizationsService::check_authorizations($this->get_item()->get_id_category())->moderation())
		{
			...
			if (!$this->get_item()->is_published())
			{
				$publication_fieldset->add_field(new FormFieldCheckbox('update_creation_date', $this->lang['form.update.creation.date'], false,
					['hidden' => $this->get_item()->get_publishing_state() != RecipeItem::NOT_PUBLISHED]
				));
			}
	...
	private function redirect()
	{
		...
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(RecipeUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title()), StringVars::replace_vars($this->lang['recipe.message.success.add'], ['title' => $item->get_title()]));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : RecipeUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $item->get_id(), $item->get_rewrited_title())), StringVars::replace_vars($this->lang['recipe.message.success.edit'], ['title' => $item->get_title()]));
		}
		else
		{
			if ($this->is_new_item)
				AppContext::get_response()->redirect(RecipeUrlBuilder::display_pending(), StringVars::replace_vars($this->lang['recipe.message.success.add'], ['title' => $item->get_title()]));
			else
				AppContext::get_response()->redirect(($this->form->get_value('referrer') ? $this->form->get_value('referrer') : RecipeUrlBuilder::display_pending()), StringVars::replace_vars($this->lang['recipe.message.success.edit'], ['title' => $item->get_title()]));
		}
	}
