...
class RecipeItem
{
	...
	{
		return CategoriesService::get_categories_manager('recipe')->get_categories_cache()->get_category($this->id_category);
	}
	...
	public function get_real_summary()
	{
		...
		return TextHelper::cut_string(@strip_tags(FormatingHelper::second_parse($this->content), '<br><br/>'), (int)RecipeConfig::load()->get_auto_cut_characters_number());
	}
	...
	public function get_persons_number()
	{
		return $this->persons_number;
	}

	public function set_persons_number($persons_number)
	{
	...
	public function set_properties(array $properties)
	{
		...
		$notation->set_module_name('recipe');
	...
	{
		$this->id_category = $id_category;
        $this->content = RecipeConfig::load()->get_default_content();
	...
	public function get_item_url()
	{
		...
		return RecipeUrlBuilder::display($category->get_id(), $category->get_rewrited_name(), $this->id, $this->get_rewrited_title())->rel();
	}
	...
	public function get_template_vars()
	{
		...
		$rich_content = HooksService::execute_hook_display_action('recipe', $content, $this->get_properties());
		$real_summary = $this->get_real_summary();
		$user = $this->get_author_user();
		$user_group_color = User::get_group_color($user->get_groups(), $user->get_level(), true);
		$comments_number = CommentsService::get_comments_number('recipe', $this->id);
		$ingredients = $this->get_ingredients();
		$ingredients_nbr = count($ingredients);
		$steps = $this->get_steps();
		$steps_nbr = count($steps);
		...
		return array_merge(
			...
			array(
				...
				'C_INGREDIENTS'          => $ingredients_nbr > 0,
				'C_STEPS'                => $steps_nbr > 0,
				'C_SEVERAL_PERSONS'      => $this->get_persons_number() > 1,
				'C_DIFFERED'             => $this->published == self::DEFERRED_PUBLICATION,
				'C_NEW_CONTENT'          => ContentManagementConfig::load()->module_new_content_is_enabled_and_check_date('recipe', $this->get_publishing_start_date() != null ? $this->get_publishing_start_date()->get_timestamp() : $this->get_creation_date()->get_timestamp()) && $this->is_published(),
				...
				'L_COMMENTS'      => CommentsService::get_lang_comments('recipe', $this->id),
				...
				'U_EDIT_CATEGORY'      => $category->get_id() == Category::ROOT_CATEGORY ? RecipeUrlBuilder::configuration()->rel() : CategoriesUrlBuilder::edit($category->get_id(), 'recipe')->rel(),
